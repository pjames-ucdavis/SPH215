...
```{r, message=FALSE}
download.file("https://raw.githubusercontent.com/pjames-ucdavis/SPH215/refs/heads/main/fast_food_davis_ca.csv", 
              destfile = "fast_food_davis_ca.csv", 
              mode = "wb")
...
First, let's set this up to be centered around the building we all know and love, the one that we are in right now. That's right--Hart Hall. I've clearly memorized the longitude and latitude for this building, which we paste in below, and then store as an **sf** object.
```{r}
loc <- c("lon" = -121.750763, "lat" = 38.540566)
loc_sf <- st_sf(name = "Hart Hall",
                geometry = st_sfc(st_point(loc), crs = 4326))
```

\

Next, we use the `osrmIsochrone()` function to create the isochrones. We will specify the *loc* to be *loc_sf*, the *breaks* (in minutes) will be 0 to 30 minutes by 5 minute intervals, and the *osrm.profile*, or the mode, will be "bike". Let's run it!
```{r}
...
# Plot using tmap
tm <- tm_shape(iso) +
  tm_polygons(fill = "run_times",
              fill.scale = tm_scale(values = rev(heat.colors(6))),  # reverse for same effect
              col = "black",
              lwd = 0.5,
              fill_alpha = 0.3,
              fill.legend = tm_legend(title = "Biking Duration")) +
  tm_shape(loc_sf) +
