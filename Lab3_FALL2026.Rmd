---
title: 'Lab 3: More with Vector Data and Visualizing Raster Data'
---
...
The objectives of this guide are to teach you how to:
   ...
   1. Create Publication-Ready Maps with vector data
...
Use the `get_acs()` command to bring in California tract-level race/ethnicity counts, total population, and total number of households. How did I find the variable IDs? Check [Lab 2](Lab2_FALL2026.html). Since we want tracts, we’ll use the `geography = "tract"` argument.
```{r, results=FALSE}
...
Unlike before, we brought these data in without the `geometry = TRUE` option. So this is just a table. But remember, an **sf** object is a data frame, so we can use `left_join()`, which we covered in [Lab 1](Lab1_FALL2026.html), to join the files *ca.inc* and *ca.tracts*.
...
Because **sf** is tidy friendly, it is no surprise we can use the **tidyverse** plotting function `ggplot()` to make maps. We already received an introduction to `ggplot()` in [Lab 2](Lab2_FALL2026.html). Recall its basic structure:
...
We can also customize the size of the legend, its alignment, font, etc. Check out the [documentation](https://r-tmap.github.io/tmap/articles/basics_legends) for more!
