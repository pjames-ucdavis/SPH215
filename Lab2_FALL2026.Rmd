...
```{r, message=FALSE}
packages <- c("tidygeocoder", "sf", "terra", "tidyverse", "tidycensus", "flextable", "tmap")
...
download.file("https://raw.githubusercontent.com/pjames-ucdavis/SPH215/refs/heads/main/CAdata.rds", "CAdata.rds", mode = "wb")

# Read in Cancer Dataset
cancer <- readRDS("CAdata.rds")

summary(cancer)
### MUST UPDATE TO USE THE RAW VERSION OF MAPGAMS CANCER DATA
```
...
```{r}
cancer_points <- st_as_sf(cancer, coords=c("X","Y"))
```
...
ca_projection <- "+proj=lcc +lat_1=40 +lat_2=41.66666666666666 
             +lat_0=39.33333333333334 +lon_0=-122 +x_0=2000000 
             +y_0=500000.0000000002 +ellps=GRS80 
             +datum=NAD83 +units=m +no_defs"

#Set CRS
cancer_points_crs <- st_set_crs(cancer_points, ca_projection)

#Look at dataset
summary(cancer_points_crs) 

#Check the CRS
st_crs(cancer_points_crs)
```
...
```{r}
tmap_mode("view")
cancer_map = tm_shape(cancer_points_crs) + tm_dots(size=0.5)
cancer_map
```

\

Let's play around with some of the options. We can change the color with the `fill=` option. We can make the dots smaller by specifying the `size=` option. And we can change the transparency of the points with the `fill_alpha=` option.
```{r}
cancer_map_small = tm_shape(cancer_points_crs) + 
...
Let's map the points color coded by the variable *event*, or whether or not the participant died over followup. We do that by specifying that the color (`fill=`) is based on the column *event*. We have to specify that *event* is a categorical variable with `fill.scale=tm_scale_categorical()`. 
```{r}
cancer_map_events = tm_shape(cancer_points_crs) + 
...
```{r}
cancer_map_events_rg = tm_shape(cancer_points_crs) + 
...
- `output`: The argument tells R to return a wide dataset as opposed to a long dataset (see this [vignette](https://tidyr.tidyverse.org/articles/tidy-data.html) for more info).
...
As you learned in [Lab 1](Lab1_FALL2026.html), whenever you bring in a dataset, the first thing you should always do is view it to get a sense of its structure and to make sure you got what you expected. One way of doing this is to use the `glimpse()` command
...
To save us time, I’ve uploaded a PolicyMap ([link to .csv](https://github.com/pjames-ucdavis/SPH215/blob/main/PolicyMap%20Data%202025-03-27%20192555%20UTC.csv)) on the Github for you to use in this lab. Save this file in the same folder where your Lab 1 R Markdown file resides. To read in a .csv file, first make sure that R is pointed to the folder you saved your data into. Type in `getwd()` to find out the current directory and `setwd("directory name")` to set the directory to the folder containing the data. 
...
```{r eval=FALSE}
setwd("C:\\Users\\pjames\\Documents\\UCD\\SPH215\\Labs\\Lab 2")
```
or a forward slash /.
...
We learned about the various data wrangling related functions from the **tidyverse** package in [Lab 1](Lab1_FALL2026.html). Let’s employ some of those functions to create a single county level dataset that joins the datasets we downloaded from the Census API and PolicyMap.
...
The average county median household income is $87,001. If the variable *mhhinc* contained missing values, we would have gotten *NA* as a result. To omit missing values from the calculation, you need to add `na.rm = TRUE` to `mean()`.
...
To get the mean, median, and standard deviation of county median income, its correlation with percent Hispanic, and give column labels for the variables in the resulting summary table, we type in:
...
Check out the [Data Sources](Other_2026.html) link for more links to US Government Data
