---
title: 'Lab 5: Working with Raster and Vector Data Together'
---
In this lab, we are going to work with vector and raster data, extracting raster values to point data 
...
The objectives of this guide are to teach you how to:
  ...
  2. Import our dataset with greenspace across the Bay Area
  3. Compare projections of datasets and re-project if needed
  4. Make maps
  5. Spatially join the two datasets by extracting raster values to points
...
As always, we check the file to make sure it was read correctly. Does it have a coordinate reference system?
...
OK, let's plot it like we did when we first looked at this dataset back in [Lab 4](Lab4_2026.html).
...
cancer_points_crs <- st_as_sf(cancer, coords=c("X","Y"), crs=ca_projection)
```
...
ca_transformed <-st_transform(cancer_points_crs, st_crs(ndvi_rast))  
...
```{r}
## Spatially join the cancer point data to the greenspace raster data
